<h2>Mensajes enviados<small></small></h2>
<p>Para enviar mensajes a otros usuarios.</p>
...
$section_name = "mensajes_salida";
$insertar_msj = "Nuevo mensaje";
...
if($window === 'nuevo' || $window === 'modi'){
	...
	mysql_select_db($database_bd_server, $bd_server);
	$query_bd_data = sprintf("SELECT e.IdMensajeEmisor AS id, e.MensajeEmisorFechaHora AS fecha_hora, d.IdUsuario AS receptor, d.MensajeRecibido, e.MensajeEmisor FROM mensajes_emisores AS e INNER JOIN mensajes_destinos AS d ON e.IdMensajeEmisor = d.IdMensajeEmisor WHERE e.IdMensajeEmisor = %s", GetSQLValueString($id, "int"));
...
<form method="post" name="registro" id="registro"  action="index.php?page=<?php echo $section_name?>">
  <table align="center" class="noborder" style="width: 70%; margin-left: auto; margin-right: auto">
    <tr valign="baseline">
	  ...
	  ?>
      <td nowrap align="right" valign="top">Enviar a:</td>
...
// --------------------------------------------------------------------------------
	...
	mysql_select_db($database_bd_server, $bd_server);
	$query = "SELECT mensajes_emisores.IdMensajeEmisor, mensajes_emisores.MensajeEmisorFechaHora, usuarios.nombreUsuario FROM mensajes_emisores
			  INNER JOIN mensajes_destinos ON mensajes_emisores.IdMensajeEmisor = mensajes_destinos.IdMensajeEmisor
			  INNER JOIN usuarios ON  mensajes_destinos.IdUsuario = usuarios.id_usuario WHERE mensajes_emisores.IdUsuario = " . $_SESSION['IdUsuario'] . " " . $where;
...
<input type="hidden" name="MM_insert" value="registro" />
    ...
    <table class="bordered-table zebra-striped" style="width: 700px; margin-left: auto; margin-right: auto">
      ...
      <?php } else {
	  do { ?>
        <tr id="row-<?php echo $row['IdMensajeEmisor']; ?>">
          <td><?php echo $row['nombreUsuario']?></td>
          <td><?php echo $row['MensajeEmisorFechaHora'] ?></td>
          <td>
          <?php if(overdriveControlAcceso($section_name."-ver", false)){ ?>
          <a class="btn" href="./?page=<?php echo $section_name?>&opt=modi&id=<?php echo $row['IdMensajeEmisor']; ?>">Ver</a> 
          <?php }
		   if(overdriveControlAcceso($section_name."-eliminar", false)){ ?>
		  <a class="btn" href="#" rel="delete" name="<?php echo $row['IdMensajeEmisor']; ?>">X</a></td>
           <?php } ?>
        </tr>

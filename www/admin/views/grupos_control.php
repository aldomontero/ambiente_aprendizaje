<h2>Administrar grupos<small></small></h2>
<p>Para gestionar los grupos de usuarios del ambiente de aprendizaje.</p>
...
$section_name = "grupos_control";
$insertar_msj = "Nuevo grupo";
$actualizar_msj = "Actualizar datos del grupo";
...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form1")) {
	if(strlen(trim($_POST['id'], " ")) === 0 ){
		$SQL = " INSERT INTO `grupos` (Grupo,IdUsuario) VALUES (
%s, %s)";
		$isInsert = true;
	} else {
		$SQL = "UPDATE `grupos` SET Grupo=%s,IdUsuario=%s WHERE `idGrupo` = %s;";
		$isInsert = false;
	}
...
<form method="post" name="registro" id="registro"  action="index.php?page=<?php echo $section_name?>">
  <table align="center" class="noborder" style="width: 70%; margin-left: auto; margin-right: auto">
    <tr valign="baseline">
      <td nowrap align="right" width="30%">Nombre del Grupo:</td>
      <td><input class="span7" type="text" name="Grupo" value="<?php echo $row_bd_data ? $row_bd_data['Grupo'] : ''; ?>" size="32"></td>
    </tr>
    <tr valign="baseline">
...
<fieldset>
    <label for="usuario">Buscar en:&nbsp;&nbsp;</label>
      <select name="opcion" id="opcion">
        <option value="1"<?php if(isset($_POST['opcion']) && $_POST['opcion'] == 1) echo ' selected="selected"'; ?>>Nombre del grupo</option>
      </select>
    ...
    <table class="bordered-table zebra-striped" style="width: 700px; margin-left: auto; margin-right: auto">
	  ...
	  do { ?>
        <tr id="row-<?php echo $row['idGrupo']; ?>">
          ...
          <a class="btn" href="./?page=<?php echo $section_name?>&opt=modi&id=<?php echo $row['idGrupo']; ?>">Editar</a> 
          <?php }
		   if(overdriveControlAcceso($section_name."-eliminar", false)){ ?>
		  <a class="btn" href="#" rel="delete" name="<?php echo $row['idGrupo']; ?>">X</a></td>
           <?php } ?>
        </tr>

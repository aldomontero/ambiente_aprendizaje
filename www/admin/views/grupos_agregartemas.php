<h2>Temas y grupos<small></small></h2>
<p>Para gestionar la relación entre temas y grupos.</p>
...
$section_name = "grupos_agregartemas";
$insertar_msj = "Nueva asignacion de temas";
$actualizar_msj = "Nueva asignacion de temas";
...
if ((isset($_POST["MM_form"])) && ($_POST["MM_form"] == "form1")) {
	if(strlen(trim($_POST['id'], " ")) === 0 ){
		$SQL = " INSERT INTO `grupos_temas` (IdGrupo,IdTema) VALUES (%s, %s)";
		$isInsert = true;
	} else {
		$SQL = "INSERT INTO `grupos_temas` (IdGrupo,IdTema) VALUES (%s, %s)";
		$isInsert = false;
...
<form method="post" name="registro" id="registro"  action="index.php?page=<?php echo $section_name?>&opt=modi&id=<?php echo $id ?>">
  <table align="center" class="noborder" style="width: 70%; margin-left: auto; margin-right: auto">
    <tr valign="baseline">
      ...
      <td><input class="span7" type="text" name="Grupo" value="<?php echo $row_bd_data ? $row_bd_data['Grupo'] : ''; ?>" size="32" disabled="disabled"></td>
    </tr>
    <tr valign="baseline">
      <?php 
		$query = "SELECT ug.*, u.* FROM grupos_temas AS ug INNER JOIN temas AS u ON ug.IdTema = u.IdTema WHERE idGrupo = " . $id;
        ...
        <table class="bordered-table zebra-striped" style="width: 100%; margin-left: auto; margin-right: auto">
          ...
          <?php } else {
          do { ?>
            <tr id="row-<?php echo $row_tema['IdGrupoTema']; ?>">
              <td><?php echo $row_tema['Tema']?></td>
              <td><?php echo $row_tema['TemaPublico']; ?></td>
              <td>
              <?php if(overdriveControlAcceso($section_name."-eliminar", false)){ ?>
		  <a class="btn" href="#" rel="delete" name="<?php echo $row_tema['IdGrupoTema']; ?>">X</a></td>
           <?php } ?>
            </tr>
      ...
      <td><select name="Tema">
        ...
        <option value="<?php echo $row['IdTema']?>"><?php echo $row['Tema'];?></option>
        <?php
		}
    ...
    <table class="bordered-table zebra-striped" style="width: 700px; margin-left: auto; margin-right: auto">
	  ...
	  do { ?>
        <tr id="row-<?php echo $row['IdGrupo']; ?>">
          ...
          <a class="btn" href="./?page=<?php echo $section_name?>&opt=modi&id=<?php echo $row['IdGrupo']; ?>">Asignar temas</a> 
          <?php }?>
        </tr>
